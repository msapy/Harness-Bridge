"""
multi_sv_explorer.py
Generates an interactive HTML SV Mode Shape Explorer for multiple CSV files.
"""
...
def compute_sv_data(csv_path, nperseg=4096, fs=128.0):
    ...
    print(f"Computing cross-spectral density matrix ({n_channels}x{n_channels}) for {os.path.basename(csv_path)}...")
    ...
    print(f"Computing SVD at {n_freqs} frequency bins for {os.path.basename(csv_path)}...")
    ...
    for k in range(n_freqs):
        ...
        try:
            ...
            print(f"Warning: SVD did not converge at frequency bin {k}. Skipping.")
            continue

    return f, sv, shapes, n_channels

def generate_multi_explorer(csv_paths, out_path, zero_joints=None, fixed_joints=None, nperseg=4096):
    ...
    with open(out_path, 'w', encoding='utf-8') as fh:
        fh.write(html)
    print(f"\nInteractive multi-explorer saved to: {out_path}")
...
<title>Harness Bridge — Multi-CSV SV Explorer</title>
...
<body>
<header>
  <div class="title">🌉 Harness Bridge · Multi-CSV SV Explorer</div>
...
def main():
    parser = argparse.ArgumentParser(description="Generate interactive SV Mode Shape Explorer HTML for multiple CSVs")
    parser.add_argument("--csv", nargs='+', required=True, help="List of CSV files to process")
    ...
    parser.add_argument("--out", default="multi_sv_explorer.html")
    ...
    generate_multi_explorer(args.csv, args.out, args.zero_points, args.fixed_points, args.nperseg)
